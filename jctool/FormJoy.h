...
public ref class DeviceUiState
{
public:
    String^ serialNumber;
    String^ firmware;
    ...
    String^ rumbleFileName;
    String^ rumbleTypeText;
    String^ rumbleSampleText;
    String^ rumbleLoopText;
    array<byte>^ rumbleLoadedFile;
    array<byte>^ rumbleConvertedFile;
    array<byte>^ backupSpi;
    String^ backupHash;
    String^ calibrationReadMac;
    int rumbleFileType;
    int rumbleSampleRate;
    int rumbleSamples;
    int rumbleLoopStart;
    int rumbleLoopEnd;
    int rumbleLoopWait;
    ...
    int rumbleConverted;
    bool hasDeviceInfo;
    bool allowFullRestore;
    bool calibrationReadValid;
    Image^ previewImage;
    Image^ irImage;
    Color bodyColor;
    Color buttonsColor;
    ...
    DeviceUiState()
    {
        ...
        batteryText = L"";
        temperatureText = L"";
        rumbleFileName = L"No file loaded";
        rumbleTypeText = L"";
        rumbleSampleText = L"";
        rumbleLoopText = L"0";
        backupHash = L"";
        calibrationReadMac = L"";
        rumbleFileType = 0;
        rumbleSampleRate = 0;
        rumbleSamples = 0;
        rumbleLoopStart = 0;
        rumbleLoopEnd = 0;
        rumbleLoopWait = 0;
        lowAmp = lowFrequency = highAmp = highFrequency = 10;
        rumbleConverted = 0;
        hasDeviceInfo = false;
        allowFullRestore = false;
        calibrationReadValid = false;
        previewImage = nullptr;
        irImage = nullptr;
        bodyColor = Color::Empty;
        buttonsColor = Color::Empty;
        leftGripColor = Color::Empty;
        rightGripColor = Color::Empty;
        controlValues = gcnew Dictionary<String^, Object^>();
    }
};

public ref class RumbleTaskRequest
{
public:
    IntPtr nativeDevice;
    array<byte>^ loadedFile;
    array<byte>^ convertedFile;
    int fileType;
    int sampleRate;
    int samples;
    int loopStart;
...
public ref class SpiBackupTaskRequest
{
public:
    IntPtr nativeDevice;
    String^ fileName;
...
public ref class DeviceTaskRequest
{
public:
    IntPtr nativeDevice;
    int taskKind;
...
public ref class IrTaskRequest
{
public:
    IntPtr nativeDevice;
    IntPtr nativeConfig;
...
public ref class FormJoy : public System::Windows::Forms::Form
{
    ...
    private: delegate void SessionControlTextDelegate(int slot, String^ controlName, String^ value, bool append);
    ...
    FormJoy(void)
    {
        activeSlot     = -1;
        shuttingDown   = false;
        restoringDeviceUi = false;
        compactLayoutReady = false;
        validated_backup_hash = L"";
        calibration_read_valid = false;
        calibration_read_mac = L"";
        handler_close   = 0;
        option_is_on    = 0;
        vib_file_type   = 0;
        vib_sample_rate = 0;
...
#pragma region Class Variables
    ...
    private: bool restoringDeviceUi;
    private: bool compactLayoutReady;
    private: String^ validated_backup_hash;
    private: bool calibration_read_valid;
    private: String^ calibration_read_mac;
    private: array<DeviceUiState^>^ deviceUiStates;
    private: array<Button^>^ deviceSlotButtons;
    private: array<LinkLabel^>^ taskOverviewRows;
    private: array<System::Windows::Forms::ContextMenuStrip^>^ deviceSlotMenus;
    private: Panel^ deviceStripPanel;
    private: Panel^ taskOverviewPanel;
    private: Label^ taskOverviewTitle;
    private: System::Windows::Forms::Timer^ taskOverviewTimer;
    private: array<Thread^>^ deviceWorkers;
...
#pragma region Windows Form Designer generated code
        ...
        void InitializeComponent(void)
        {
            ...
            this->lbl_spiProggressDesc->Text = L"Creating a verified SPI backup...\nThis can take around 20 minutes.\n\nDon\'t disconnect "
                L"your device!";
            ...
            this->Text = L"Joy-Con Toolkit v5.3.0 Multi-device";
...
#pragma endregion
    ...
    private: String^ deviceTypeName(int type) {
        if (type == 1) return L"Joy-Con (L)";
        if (type == 2) return L"Joy-Con (R)";
        if (type == 3) return L"Pro Controller";
        return L"Unknown device";
    }

    private: System::Void applyDeviceCapabilities(device_context *device) {
        if (!device)
            return;
        this->btn_changeGripsColor->Enabled = device->device_type == 3;
        this->iRCameraToolStripMenuItem->Enabled = device->device_type == 2;
        this->grpBox_nfc->Enabled = device->device_type != 1;
    }

    private: String^ taskTypeName(int task) {
        if (task == DEVICE_TASK_RUMBLE) return L"HD Rumble";
        if (task == DEVICE_TASK_SPI_BACKUP) return L"SPI backup";
        if (task == DEVICE_TASK_BUTTON_TEST) return L"Button test";
        if (task == DEVICE_TASK_IR) return L"IR camera";
        if (task == DEVICE_TASK_NFC) return L"NFC scan";
        if (task == DEVICE_TASK_RESTORE) return L"Full restore";
        return L"";
    }

    private: String^ formatTaskTime(unsigned long long milliseconds) {
        TimeSpan value = TimeSpan::FromMilliseconds((double)milliseconds);
        if (value.TotalHours >= 1.0)
            return String::Format(L"{0:D2}:{1:D2}:{2:D2}", (int)value.TotalHours, value.Minutes, value.Seconds);
        return String::Format(L"{0:D2}:{1:D2}", value.Minutes, value.Seconds);
    }

    private: u32 readLittleEndian32(array<byte>^ data, int offset) {
        return (u32)data[offset] | ((u32)data[offset + 1] << 8) |
            ((u32)data[offset + 2] << 16) | ((u32)data[offset + 3] << 24);
    }

    private: u32 readBigEndian32(array<byte>^ data, int offset) {
        return ((u32)data[offset] << 24) | ((u32)data[offset + 1] << 16) |
            ((u32)data[offset + 2] << 8) | (u32)data[offset + 3];
    }

    private: String^ calculateFileSha256(String^ path) {
        FileStream^ stream = File::OpenRead(path);
        System::Security::Cryptography::SHA256^ sha =
            System::Security::Cryptography::SHA256::Create();
        array<byte>^ hash = sha->ComputeHash(stream);
        delete stream;
        delete sha;
        return BitConverter::ToString(hash)->Replace(L"-", L"")->ToLowerInvariant();
    }

    private: String^ calculateBytesSha256(array<byte>^ data) {
        if (!data)
            return L"";
        System::Security::Cryptography::SHA256^ sha =
            System::Security::Cryptography::SHA256::Create();
        array<byte>^ hash = sha->ComputeHash(data);
        delete sha;
        return BitConverter::ToString(hash)->Replace(L"-", L"")->ToLowerInvariant();
    }

    private: System::Void writeBackupChecksum(String^ path) {
        File::WriteAllText(path + L".sha256", calculateFileSha256(path));
    }

    private: bool validateBackupChecksum(String^ path, array<byte>^ loadedData) {
        String^ checksumPath = path + L".sha256";
        if (!loadedData || !File::Exists(checksumPath))
            return false;
        String^ expected = File::ReadAllText(checksumPath)->Trim();
        return expected->Length == 64 && String::Equals(expected, calculateBytesSha256(loadedData),
            StringComparison::OrdinalIgnoreCase);
    }

    private: bool loadedBackupMatchesCurrentController() {
        if (!this->backup_spi || this->backup_spi->Length != 524288 ||
            this->backup_spi[0x6012] != handle_ok)
            return false;
        unsigned char deviceInfo[10];
        memset(deviceInfo, 0, sizeof(deviceInfo));
        if (get_device_info(deviceInfo) != 0)
            return false;
        for (int i = 4; i < 10; ++i) {
            if (deviceInfo[i] != this->backup_spi[0x1A - i + 4])
                return false;
        }
        return true;
    }

    private: bool loadedBackupHasValidChecksum() {
        return this->backup_spi && this->backup_spi->Length == 524288 &&
            !String::IsNullOrWhiteSpace(validated_backup_hash) &&
            String::Equals(validated_backup_hash, calculateBytesSha256(this->backup_spi),
                StringComparison::OrdinalIgnoreCase);
    }

    private: bool calibrationStateMatchesCurrentController() {
        if (!calibration_read_valid || String::IsNullOrWhiteSpace(calibration_read_mac))
            return false;
        unsigned char deviceInfo[10];
        memset(deviceInfo, 0, sizeof(deviceInfo));
        if (get_device_info(deviceInfo) != 0)
            return false;
        String^ currentMac = String::Format("{0:X2}:{1:X2}:{2:X2}:{3:X2}:{4:X2}:{5:X2}",
            deviceInfo[4], deviceInfo[5], deviceInfo[6], deviceInfo[7], deviceInfo[8], deviceInfo[9]);
        return String::Equals(calibration_read_mac, currentMac, StringComparison::OrdinalIgnoreCase);
    }

    private: System::Void updateCompactLayout(int runningTasks) {
        if (!compactLayoutReady || !this->IsHandleCreated)
            return;

        int contentRight = 0;
        int contentBottom = 0;
        for each (Control^ control in this->Controls) {
            if (!control->Visible || control == this->menuStrip1 || control == this->toolStrip1 ||
                control == deviceStripPanel || control == taskOverviewPanel || control == panel_filler)
                continue;
            contentRight = Math::Max(contentRight, control->Right);
            contentBottom = Math::Max(contentBottom, control->Bottom);
        }

        int clientWidth = Math::Max(760, contentRight + 16);
        int taskHeight = runningTasks > 0 ? 91 : 36;
        int taskTop = Math::Max(220, contentBottom + 12);
        int clientHeight = taskTop + taskHeight + this->toolStrip1->Height + 8;
        deviceStripPanel->Size = System::Drawing::Size(clientWidth - 16, 78);
        taskOverviewPanel->Location = Point(8, taskTop);
        taskOverviewPanel->Size = System::Drawing::Size(clientWidth - 16, taskHeight);
        taskOverviewTitle->Size = System::Drawing::Size(clientWidth - 36, 20);

        int slotWidth = (deviceStripPanel->Width - 27) / 4;
        for (int i = 0; i < 4; ++i) {
            deviceSlotButtons[i]->Location = Point(6 + i * (slotWidth + 5), 6);
            deviceSlotButtons[i]->Size = System::Drawing::Size(slotWidth, 66);
            taskOverviewRows[i]->Size = System::Drawing::Size(clientWidth - 36, 16);
        }

        System::Drawing::Size desiredSize = System::Drawing::Size(clientWidth, clientHeight);
        if (this->ClientSize != desiredSize)
            this->ClientSize = desiredSize;
    }

    private: System::Void initializeMultiDeviceUi() {
        deviceUiStates = gcnew array<DeviceUiState^>(4);
        deviceSlotButtons = gcnew array<Button^>(4);
        taskOverviewRows = gcnew array<LinkLabel^>(4);
        deviceSlotMenus = gcnew array<System::Windows::Forms::ContextMenuStrip^>(4);
        deviceWorkers = gcnew array<Thread^>(4);
        for (int i = 0; i < 4; ++i)
            deviceUiStates[i] = gcnew DeviceUiState();

        this->SuspendLayout();
        for each (Control^ control in this->Controls) {
            if (control != this->menuStrip1 && control != this->toolStrip1 && control->Top >= 36)
                control->Top += 88;
        }
        ...
        this->ClientSize = System::Drawing::Size(760, 600);
        this->BackColor = Color::FromArgb(34, 37, 41);
        this->textBoxDev->Size = System::Drawing::Size(220, 25);
        this->label_samples->MaximumSize = System::Drawing::Size(196, 0);
        this->label_samples->Font = gcnew System::Drawing::Font(L"Segoe UI", 8.25F);

        deviceStripPanel = gcnew Panel();
        deviceStripPanel->Name = L"deviceStripPanel";
        deviceStripPanel->Location = Point(8, 28);
        deviceStripPanel->Size = System::Drawing::Size(744, 78);
        deviceStripPanel->Anchor = static_cast<AnchorStyles>(
            AnchorStyles::Top | AnchorStyles::Left | AnchorStyles::Right);
        deviceStripPanel->BackColor = Color::FromArgb(25, 28, 32);

        for (int i = 0; i < 4; ++i) {
            Button^ slotButton = gcnew Button();
            slotButton->Name = String::Format(L"deviceSlot{0}", i + 1);
            slotButton->Tag = i;
            slotButton->Location = Point(6 + i * 183, 6);
            slotButton->Size = System::Drawing::Size(178, 66);
            slotButton->FlatStyle = FlatStyle::Flat;
            slotButton->FlatAppearance->BorderSize = 1;
            slotButton->FlatAppearance->BorderColor = Color::FromArgb(105, 105, 105);
            slotButton->BackColor = Color::FromArgb(43, 47, 52);
            slotButton->ForeColor = Color::FromArgb(238, 241, 244);
            slotButton->TextAlign = ContentAlignment::MiddleLeft;
            slotButton->Font = gcnew System::Drawing::Font(L"Segoe UI", 8.5F, FontStyle::Regular);
            slotButton->AutoEllipsis = true;
            slotButton->Click += gcnew EventHandler(this, &FormJoy::deviceSlot_Click);
            System::Windows::Forms::ContextMenuStrip^ slotMenu =
                gcnew System::Windows::Forms::ContextMenuStrip();
            ToolStripMenuItem^ identifyItem = gcnew ToolStripMenuItem(L"Identify controller");
            identifyItem->Tag = i;
            identifyItem->Click += gcnew EventHandler(this, &FormJoy::identifyDevice_Click);
            ToolStripMenuItem^ renameItem = gcnew ToolStripMenuItem(L"Rename slot");
            renameItem->Tag = i;
            renameItem->Click += gcnew EventHandler(this, &FormJoy::renameDevice_Click);
            slotMenu->Items->Add(identifyItem);
            slotMenu->Items->Add(renameItem);
            slotButton->ContextMenuStrip = slotMenu;
            deviceSlotButtons[i] = slotButton;
            deviceSlotMenus[i] = slotMenu;
            deviceStripPanel->Controls->Add(slotButton);
        }

        taskOverviewPanel = gcnew Panel();
        taskOverviewPanel->Name = L"taskOverviewPanel";
        taskOverviewPanel->Location = Point(8, 550);
        taskOverviewPanel->Size = System::Drawing::Size(744, 36);
        taskOverviewPanel->Anchor = static_cast<AnchorStyles>(
            AnchorStyles::Left | AnchorStyles::Right);
        taskOverviewPanel->BackColor = Color::FromArgb(25, 28, 32);

        taskOverviewTitle = gcnew Label();
        taskOverviewTitle->Location = Point(10, 7);
        taskOverviewTitle->Size = System::Drawing::Size(720, 20);
        taskOverviewTitle->ForeColor = Color::FromArgb(69, 220, 180);
        taskOverviewTitle->Font = gcnew System::Drawing::Font(L"Segoe UI Semibold", 9.0F);
        taskOverviewPanel->Controls->Add(taskOverviewTitle);

        for (int i = 0; i < 4; ++i) {
            LinkLabel^ taskRow = gcnew LinkLabel();
            taskRow->Name = String::Format(L"taskOverviewRow{0}", i + 1);
            taskRow->Tag = i;
            taskRow->Location = Point(10, 28 + i * 15);
            taskRow->Size = System::Drawing::Size(720, 16);
            taskRow->LinkColor = Color::FromArgb(220, 225, 230);
            taskRow->ActiveLinkColor = Color::FromArgb(69, 220, 180);
            taskRow->VisitedLinkColor = taskRow->LinkColor;
            taskRow->LinkClicked += gcnew LinkLabelLinkClickedEventHandler(this, &FormJoy::taskOverviewRow_LinkClicked);
            taskOverviewRows[i] = taskRow;
            taskOverviewPanel->Controls->Add(taskRow);
        }

        this->Controls->Add(deviceStripPanel);
        this->Controls->Add(taskOverviewPanel);
        deviceStripPanel->BringToFront();
        taskOverviewPanel->BringToFront();

        taskOverviewTimer = gcnew System::Windows::Forms::Timer();
        taskOverviewTimer->Interval = 250;
        taskOverviewTimer->Tick += gcnew EventHandler(this, &FormJoy::taskOverviewTimer_Tick);
        this->Shown += gcnew EventHandler(this, &FormJoy::FormJoy_Shown);
        taskOverviewTimer->Start();
        refreshDeviceStatusUi();
        this->ResumeLayout(false);
    }

    private: System::Void FormJoy_Shown(System::Object^ sender, System::EventArgs^ e) {
        refreshDeviceStatusUi();
    }

    private: System::Void captureControlValues(Control^ root, Dictionary<String^, Object^>^ values) {
        if (!String::IsNullOrEmpty(root->Name)) {
            if (dynamic_cast<TextBox^>(root))
                values[root->Name] = safe_cast<TextBox^>(root)->Text;
            else if (dynamic_cast<NumericUpDown^>(root))
                values[root->Name] = safe_cast<NumericUpDown^>(root)->Value;
            else if (dynamic_cast<TrackBar^>(root))
                values[root->Name] = safe_cast<TrackBar^>(root)->Value;
            else if (dynamic_cast<CheckBox^>(root))
                values[root->Name] = safe_cast<CheckBox^>(root)->Checked;
            else if (dynamic_cast<RadioButton^>(root))
                values[root->Name] = safe_cast<RadioButton^>(root)->Checked;
            else if (dynamic_cast<ComboBox^>(root))
                values[root->Name] = safe_cast<ComboBox^>(root)->SelectedIndex;
            else if (dynamic_cast<Label^>(root))
                values[root->Name] = safe_cast<Label^>(root)->Text;
        }
        for each (Control^ child in root->Controls)
            captureControlValues(child, values);
    }

    private: System::Void restoreControlValues(Control^ root, Dictionary<String^, Object^>^ values) {
        Object^ value;
        if (!String::IsNullOrEmpty(root->Name) && values->TryGetValue(root->Name, value)) {
            if (dynamic_cast<TextBox^>(root))
                safe_cast<TextBox^>(root)->Text = safe_cast<String^>(value);
            else if (dynamic_cast<NumericUpDown^>(root)) {
                NumericUpDown^ numeric = safe_cast<NumericUpDown^>(root);
                Decimal number = safe_cast<Decimal>(value);
                numeric->Value = Math::Min(numeric->Maximum, Math::Max(numeric->Minimum, number));
            }
            else if (dynamic_cast<TrackBar^>(root)) {
                TrackBar^ track = safe_cast<TrackBar^>(root);
                track->Value = Math::Min(track->Maximum, Math::Max(track->Minimum, safe_cast<int>(value)));
            }
            else if (dynamic_cast<CheckBox^>(root))
                safe_cast<CheckBox^>(root)->Checked = safe_cast<bool>(value);
            else if (dynamic_cast<RadioButton^>(root))
                safe_cast<RadioButton^>(root)->Checked = safe_cast<bool>(value);
            else if (dynamic_cast<ComboBox^>(root)) {
                ComboBox^ combo = safe_cast<ComboBox^>(root);
                int index = safe_cast<int>(value);
                if (index >= -1 && index < combo->Items->Count)
                    combo->SelectedIndex = index;
            }
            else if (dynamic_cast<Label^>(root))
                safe_cast<Label^>(root)->Text = safe_cast<String^>(value);
        }
        for each (Control^ child in root->Controls)
            restoreControlValues(child, values);
    }

    private: array<Control^>^ deviceStateRoots() {
        return gcnew array<Control^> {
            this->grpBox_Color, this->grpBox_DebugCmd, this->grpBox_Restore,
            this->grpBox_ChangeSN, this->grpBox_VibPlayer, this->grpBox_ButtonTest,
            this->grpBox_IR, this->grpBox_IRSettings, this->grpBox_editCalModel,
            this->grpBox_StickCal, this->grpBox_dev_param, this->grpBox_accGyroCal,
            this->grpBox_nfc
        };
    }

    private: System::Void saveActiveDeviceState() {
        if (activeSlot < 0 || activeSlot >= 4)
            return;
        DeviceUiState^ state = deviceUiStates[activeSlot];
        state->serialNumber = this->textBoxSN->Text;
        state->firmware = this->textBoxFW->Text;
        state->macAddress = this->textBoxMAC->Text;
        state->deviceName = this->textBoxDev->Text;
        state->batteryText = this->toolStripLabel_batt->Text;
        state->temperatureText = this->toolStripLabel_temp->Text;
        state->rumbleFileName = this->label_hdrumble_filename->Text;
        state->rumbleTypeText = this->label_vib_loaded->Text;
        state->rumbleSampleText = this->label_samples->Text;
        state->rumbleLoopText = this->textBox_vib_loop_times->Text;
        state->rumbleLoadedFile = this->vib_loaded_file;
        state->rumbleConvertedFile = this->vib_file_converted;
        state->backupSpi = this->backup_spi;
        state->backupHash = validated_backup_hash;
        state->rumbleFileType = vib_file_type;
        state->rumbleSampleRate = vib_sample_rate;
        state->rumbleSamples = vib_samples;
        state->rumbleLoopStart = vib_loop_start;
        state->rumbleLoopEnd = vib_loop_end;
        state->rumbleLoopWait = vib_loop_wait;
        state->rumbleConverted = vib_converted;
        state->lowAmp = this->trackBar_lf_amp->Value;
        state->lowFrequency = this->trackBar_lf_freq->Value;
        state->highAmp = this->trackBar_hf_amp->Value;
        state->highFrequency = this->trackBar_hf_freq->Value;
        state->previewImage = this->pictureBoxPreview->Image;
        state->irImage = this->pictureBoxIR->Image;
        state->bodyColor = jcBodyColor;
        state->buttonsColor = jcButtonsColor;
        state->leftGripColor = jcGripLeftColor;
        state->rightGripColor = jcGripRightColor;
        state->allowFullRestore = allow_full_restore;
        state->calibrationReadValid = calibration_read_valid;
        state->calibrationReadMac = calibration_read_mac;
        state->hasDeviceInfo = device_connection() != 0;
        for each (Control^ root in deviceStateRoots())
            captureControlValues(root, state->controlValues);
    }

    private: System::Void loadDeviceState(int slot) {
        DeviceUiState^ state = deviceUiStates[slot];
        restoringDeviceUi = true;
        this->textBoxSN->Text = state->serialNumber;
        this->textBoxFW->Text = state->firmware;
        this->textBoxMAC->Text = state->macAddress;
        this->textBoxDev->Text = state->deviceName;
        this->toolStripLabel_batt->Text = state->batteryText;
        this->toolStripLabel_temp->Text = state->temperatureText;
        this->label_hdrumble_filename->Text = state->rumbleFileName;
        this->label_vib_loaded->Text = state->rumbleTypeText;
        this->label_samples->Text = state->rumbleSampleText;
        this->textBox_vib_loop_times->Text = state->rumbleLoopText;
        this->vib_loaded_file = state->rumbleLoadedFile;
        this->vib_file_converted = state->rumbleConvertedFile;
        this->backup_spi = state->backupSpi;
        validated_backup_hash = state->backupHash;
        vib_file_type = state->rumbleFileType;
        vib_sample_rate = (u16)state->rumbleSampleRate;
        vib_samples = (u32)state->rumbleSamples;
        vib_loop_start = (u32)state->rumbleLoopStart;
        vib_loop_end = (u32)state->rumbleLoopEnd;
        vib_loop_wait = (u32)state->rumbleLoopWait;
        this->trackBar_lf_amp->Value = state->lowAmp;
        this->trackBar_lf_freq->Value = state->lowFrequency;
        this->trackBar_hf_amp->Value = state->highAmp;
        this->trackBar_hf_freq->Value = state->highFrequency;
        vib_converted = state->rumbleConverted;
        if (state->previewImage)
            this->pictureBoxPreview->Image = state->previewImage;
        if (state->irImage)
            this->pictureBoxIR->Image = state->irImage;
        if (!state->bodyColor.IsEmpty)
            jcBodyColor = state->bodyColor;
        if (!state->buttonsColor.IsEmpty)
            jcButtonsColor = state->buttonsColor;
        if (!state->leftGripColor.IsEmpty)
            jcGripLeftColor = state->leftGripColor;
        if (!state->rightGripColor.IsEmpty)
            jcGripRightColor = state->rightGripColor;
        allow_full_restore = state->allowFullRestore;
        calibration_read_valid = state->calibrationReadValid;
        calibration_read_mac = state->calibrationReadMac;
        for each (Control^ root in deviceStateRoots())
            restoreControlValues(root, state->controlValues);
        this->btn_writeStickParams->Enabled = calibration_read_valid;
        this->btn_writeUserCal->Enabled = calibration_read_valid;

        device_context *device = get_device_slot(slot);
        bool playing = device && device->task_kind == DEVICE_TASK_RUMBLE;
        this->btn_vibPlay->Text = playing ? (device->cancel_requested ? L"Stopping..." : L"Stop") : L"Play";
        this->btn_vibPlay->Enabled = playing || vib_file_type != 0;
        this->btn_loadVib->Enabled = !playing;
        this->groupBox_vib_eq->Enabled = !playing && vib_file_type != 1;
        restoringDeviceUi = false;
    }

    private: System::Void showNoDeviceSelected() {
        activeSlot = -1;
        if (compactLayoutReady)
            reset_window_option(true);
        this->textBoxSN->Text = L"";
        this->textBoxFW->Text = L"";
        this->textBoxMAC->Text = L"";
        validated_backup_hash = L"";
        calibration_read_valid = false;
        calibration_read_mac = L"";
        this->btn_writeStickParams->Enabled = false;
        this->btn_writeUserCal->Enabled = false;
        this->textBoxDev->Text = L"No controller selected";
        this->grpBox_Color->Enabled = false;
        this->grpBox_Color->Visible = false;
        this->grpBox_SPI->Visible = false;
        this->toolStripBtn_Disconnect->Enabled = false;
    }

    private: System::Void selectDeviceSlot(int slot) {
        if (handler_close != 0 && slot != activeSlot)
            return;
        device_context *device = get_device_slot(slot);
        if (!device || !device->connected)
            return;
        ...
        this->SuspendLayout();
        deviceStripPanel->SuspendLayout();
        saveActiveDeviceState();
        activeSlot = slot;
        set_active_device_context(device);
        applyDeviceCapabilities(device);
        loadDeviceState(slot);
        ...
        if (!device->initialized && device->task_kind == DEVICE_TASK_NONE) {
            silence_input_report();
            set_led_busy();
            device->initialized = true;
        }
        if (device->task_kind == DEVICE_TASK_NONE) {
            full_refresh(false);
            saveActiveDeviceState();
            device->task_result = 0;
        }
        refreshDeviceStatusUi();
        deviceStripPanel->ResumeLayout(false);
        this->ResumeLayout(false);
        this->Invalidate(true);
    }

    private: System::Void refreshAllDeviceCaches(int preferredSlot) {
        ...
        device_context *preferredDevice = get_device_slot(preferredSlot);
        int targetSlot = preferredDevice && preferredDevice->connected ? preferredSlot : firstConnectedSlot;
        if (targetSlot >= 0) {
            device_context *targetDevice = get_device_slot(targetSlot);
            activeSlot = targetSlot;
            set_active_device_context(targetDevice);
            applyDeviceCapabilities(targetDevice);
            loadDeviceState(targetSlot);
            this->grpBox_Color->Enabled = true;
            if (option_is_on == 0) {
                this->grpBox_Color->Visible = true;
                this->grpBox_SPI->Visible = false;
            ...
            set_active_device_context(nullptr);
            showNoDeviceSelected();
        }
        refreshDeviceStatusUi();
        deviceStripPanel->ResumeLayout(false);
    ...
    public: bool runGlobalCacheSelfTest() {
        int preferredSlot = activeSlot;
        refresh_device_sessions();
        refreshAllDeviceCaches(preferredSlot);
        for (int i = 0; i < 4; ++i) {
            device_context *device = get_device_slot(i);
            DeviceUiState^ state = deviceUiStates[i];
            if (!device || !device->connected || !state->hasDeviceInfo ||
                String::IsNullOrWhiteSpace(state->deviceName) ||
                String::IsNullOrWhiteSpace(state->macAddress))
                return false;
        }
        this->Show();
        ...
        refreshDeviceStatusUi();
        Application::DoEvents();
        System::Drawing::Size stableClientSize = this->ClientSize;
        System::Drawing::Size stableDeviceStripSize = deviceStripPanel->Size;
        System::Drawing::SizeF stableAutoScaleDimensions = this->AutoScaleDimensions;
        bool slotSwitchStable = true;
        for (int i = 0; i < 4; ++i) {
            selectDeviceSlot(i);
            Application::DoEvents();
            if (this->ClientSize != stableClientSize ||
                deviceStripPanel->Size != stableDeviceStripSize ||
                this->AutoScaleDimensions != stableAutoScaleDimensions) {
                slotSwitchStable = false;
                break;
            }
        }
        bool compact = this->ClientSize.Width <= 1200 && this->ClientSize.Height <= 1100 &&
            deviceStripPanel->Right <= this->ClientSize.Width &&
            taskOverviewPanel->Top - this->grpBox_Color->Bottom <= 20 &&
            this->toolStrip1->Top - taskOverviewPanel->Bottom <= 20 && slotSwitchStable;
        this->Hide();
        return compact;
    }

    private: System::Void deviceSlot_Click(System::Object^ sender, System::EventArgs^ e) {
        if (handler_close != 0) {
            MessageBox::Show(L"This controller is in a non-interruptible restore operation.",
                L"Operation in progress", MessageBoxButtons::OK, MessageBoxIcon::Information);
            return;
        }
        Button^ button = safe_cast<Button^>(sender);
        selectDeviceSlot(safe_cast<int>(button->Tag));
    }

    private: System::Void identifyDevice_Click(System::Object^ sender, System::EventArgs^ e) {
        ToolStripMenuItem^ item = safe_cast<ToolStripMenuItem^>(sender);
        int slot = safe_cast<int>(item->Tag);
        device_context *device = get_device_slot(slot);
        if (!device || !device->connected)
            return;
        if (device->task_kind != DEVICE_TASK_NONE) {
            MessageBox::Show(L"This controller is busy and cannot run the identify vibration.",
                L"Controller busy", MessageBoxButtons::OK, MessageBoxIcon::Information);
            return;
        }
        device_context *previous = current_device_context();
        set_thread_device_context(device);
        send_rumble();
        set_thread_device_context(previous);
    }

    private: System::Void renameDevice_Click(System::Object^ sender, System::EventArgs^ e) {
        ToolStripMenuItem^ item = safe_cast<ToolStripMenuItem^>(sender);
        int slot = safe_cast<int>(item->Tag);
        Form^ dialog = gcnew Form();
        dialog->Text = L"Rename device slot";
        dialog->FormBorderStyle = System::Windows::Forms::FormBorderStyle::FixedDialog;
        dialog->StartPosition = FormStartPosition::CenterParent;
        dialog->ClientSize = System::Drawing::Size(320, 92);
        dialog->MinimizeBox = false;
        dialog->MaximizeBox = false;

        TextBox^ nameInput = gcnew TextBox();
        nameInput->Location = Point(12, 12);
        nameInput->Size = System::Drawing::Size(296, 25);
        nameInput->MaxLength = 24;
        nameInput->Text = deviceUiStates[slot]->customName;
        Button^ okButton = gcnew Button();
        okButton->Text = L"OK";
        okButton->DialogResult = System::Windows::Forms::DialogResult::OK;
        okButton->Location = Point(152, 51);
        Button^ cancelButton = gcnew Button();
        cancelButton->Text = L"Cancel";
        cancelButton->DialogResult = System::Windows::Forms::DialogResult::Cancel;
        cancelButton->Location = Point(233, 51);
        ...
        if (dialog->ShowDialog(this) == System::Windows::Forms::DialogResult::OK) {
            deviceUiStates[slot]->customName = nameInput->Text->Trim();
            refreshDeviceStatusUi();
        }
        delete dialog;
    }

    private: System::Void refreshDeviceStatusUi() {
        int runningTasks = 0;
        int taskRowIndex = 0;
        ...
        for (int i = 0; i < 4; ++i) {
            device_context *device = get_device_slot(i);
            Button^ slotButton = deviceSlotButtons[i];
            if (!device) {
                slotButton->Text = String::Format(L"{0}  AVAILABLE\r\nWaiting for controller", i + 1);
                slotButton->Enabled = true;
            }
            else {
                String^ identity = deviceUiStates[i]->macAddress;
                if (String::IsNullOrEmpty(identity) && !device->serial.empty())
                    identity = gcnew String(device->serial.c_str());
                if (String::IsNullOrEmpty(identity))
                    identity = L"Unknown ID";
                if (identity->Length > 8)
                    identity = identity->Substring(identity->Length - 8);
                String^ bodyColorText = deviceUiStates[i]->bodyColor.IsEmpty
                    ? L""
                    : String::Format(L"#{0:X2}{1:X2}{2:X2}",
                        deviceUiStates[i]->bodyColor.R, deviceUiStates[i]->bodyColor.G,
                        deviceUiStates[i]->bodyColor.B);
                String^ batteryText = deviceUiStates[i]->batteryText;
                if (!String::IsNullOrWhiteSpace(batteryText) && batteryText->Contains(L"-"))
                    batteryText = batteryText->Substring(batteryText->LastIndexOf(L'-') + 1)->Trim();
                if (String::IsNullOrWhiteSpace(batteryText))
                    batteryText = L"--";

                String^ status = device->connected ? L"Ready" : L"Disconnected";
                int task = device->task_kind;
                if (task != DEVICE_TASK_NONE) {
                    int progress = device->task_progress.load();
                    status = progress > 0
                        ? String::Format(L"{0}  {1}%", taskTypeName(task), progress)
                        : taskTypeName(task) + L"  running";
                }
                else if (device->task_result == 1)
                    status = L"Task completed";
                else if (device->task_result == 2)
                    status = L"Task stopped";
                else if (device->task_result == 3)
                    status = L"Task failed";
                if (i == activeSlot)
                    status = L"CURRENT  |  " + status;

                String^ displayName = String::IsNullOrWhiteSpace(deviceUiStates[i]->customName)
                    ? deviceTypeName(device->device_type)
                    : deviceUiStates[i]->customName;
                slotButton->Text = String::Format(L"{0}  {1}  {2}\r\n{3}  {4}\r\n{5}",
                    i + 1, displayName, batteryText, identity, bodyColorText, status);
                slotButton->Enabled = true;
            }

            bool selected = i == activeSlot;
            slotButton->FlatAppearance->BorderSize = selected ? 2 : 1;
            slotButton->FlatAppearance->BorderColor = selected
                ? Color::FromArgb(69, 220, 180) : Color::FromArgb(82, 88, 94);
            slotButton->BackColor = selected ? Color::FromArgb(53, 61, 66) : Color::FromArgb(43, 47, 52);

            if (device && device->task_kind != DEVICE_TASK_NONE) {
                int task = device->task_kind;
                unsigned long long elapsed = device->task_started_ms ? now - device->task_started_ms : 0;
                String^ timing = L"elapsed " + formatTaskTime(elapsed);
                if (device->task_total_ms > 0 && elapsed < device->task_total_ms)
                    timing = L"remaining " + formatTaskTime(device->task_total_ms - elapsed);
                String^ progressText = device->task_progress.load() > 0
                    ? String::Format(L"{0}%", device->task_progress.load())
                    : L"running";
                String^ taskDeviceName = String::IsNullOrWhiteSpace(deviceUiStates[i]->customName)
                    ? String::Format(L"Device {0}", i + 1)
                    : String::Format(L"Device {0} ({1})", i + 1, deviceUiStates[i]->customName);
                taskOverviewRows[taskRowIndex]->Tag = i;
                taskOverviewRows[taskRowIndex]->Text = String::Format(
                    L"{0}  |  {1}  |  {2}  |  {3}",
                    taskDeviceName, taskTypeName(task), progressText, timing);
                taskOverviewRows[taskRowIndex]->Visible = true;
                taskRowIndex++;
                runningTasks++;
            }
        }

        for (int i = taskRowIndex; i < 4; ++i)
            taskOverviewRows[i]->Visible = false;
        taskOverviewTitle->Text = runningTasks == 0
            ? L"No running tasks"
            : String::Format(L"Running tasks  {0}", runningTasks);
        updateCompactLayout(runningTasks);
    }

    private: System::Void taskOverviewTimer_Tick(System::Object^ sender, System::EventArgs^ e) {
        if (shuttingDown)
            return;
        refreshDeviceStatusUi();
        if (activeSlot >= 0) {
            device_context *device = get_device_slot(activeSlot);
            Image^ latestIrImage = deviceUiStates[activeSlot]->irImage;
            if (latestIrImage && this->pictureBoxIR->Image != latestIrImage) {
                Image^ previousImage = this->pictureBoxIR->Image;
                this->pictureBoxIR->Image = latestIrImage;
                this->pictureBoxIR->ClientSize = System::Drawing::Size(240, 320);
                bool cachedImage = false;
                for (int i = 0; i < 4; ++i) {
                    if (deviceUiStates[i]->irImage == previousImage) {
                        cachedImage = true;
                        break;
                    }
                }
                if (previousImage && !cachedImage)
                    delete previousImage;
            }
            if (device && device->task_kind == DEVICE_TASK_RUMBLE) {
                this->btn_vibPlay->Text = device->cancel_requested ? L"Stopping..." : L"Stop";
                this->btn_vibPlay->Enabled = !device->cancel_requested;
            }
            else if (device && option_is_on == 4) {
                this->btn_vibPlay->Text = L"Play";
                this->btn_vibPlay->Enabled = vib_file_type != 0;
                this->btn_loadVib->Enabled = true;
                this->groupBox_vib_eq->Enabled = vib_file_type != 1;
            }
            if (device && device->task_kind == DEVICE_TASK_SPI_BACKUP) {
                this->label_progress->Text = String::Format(L"{0}%", device->task_progress.load());
                this->btn_spiCancel->Enabled = !device->cancel_requested;
            }
            if (device && device->task_kind == DEVICE_TASK_BUTTON_TEST) {
                this->btn_runBtnTest->Text = device->cancel_requested ? L"Stopping..." : L"Turn off";
                this->btn_runBtnTest->Enabled = !device->cancel_requested;
            }
            else if (device && option_is_on == 5) {
                this->btn_runBtnTest->Text = L"Turn on";
                this->btn_runBtnTest->Enabled = true;
            }
            if (device && device->task_kind == DEVICE_TASK_NFC) {
                this->btn_NFC->Text = device->cancel_requested ? L"Stopping..." : L"Stop";
                this->btn_NFC->Enabled = !device->cancel_requested;
            }
            else if (device && option_is_on == 5) {
                this->btn_NFC->Text = L"Scan";
                this->btn_NFC->Enabled = true;
            }
            if (device && device->task_kind == DEVICE_TASK_IR) {
                this->btn_getIRStream->Text = device->cancel_requested
                    ? L"Stopping..."
                    : (device->enable_ir_video_photo_state ? L"Stop" : L"Cancel");
                this->btn_getIRStream->Enabled = !device->cancel_requested;
                this->btn_getIRImage->Enabled = false;
                this->grpBox_IRRes->Enabled = false;
                this->btn_IRConfigLive->Enabled = device->enable_ir_video_photo_state && !device->cancel_requested;
            }
            else if (device && option_is_on == 6) {
                this->btn_getIRStream->Text = L"Stream";
    ...
    private: System::Void taskOverviewRow_LinkClicked(System::Object^ sender, LinkLabelLinkClickedEventArgs^ e) {
        LinkLabel^ row = safe_cast<LinkLabel^>(sender);
        int slot = safe_cast<int>(row->Tag);
        selectDeviceSlot(slot);
        device_context *device = get_device_slot(slot);
        if (!device)
            return;
        if (device->task_kind == DEVICE_TASK_RUMBLE) {
            reset_window_option(false);
            this->Controls->Add(this->grpBox_VibPlayer);
            option_is_on = 4;
        }
        else if (device->task_kind == DEVICE_TASK_SPI_BACKUP) {
            reset_window_option(false);
            this->grpBox_Color->Visible = false;
            this->grpBox_SPI->Visible = true;
        }
        else if (device->task_kind == DEVICE_TASK_BUTTON_TEST) {
            reset_window_option(false);
            this->Controls->Add(this->grpBox_ButtonTest);
            this->Controls->Add(this->grpBox_StickCal);
            this->Controls->Add(this->grpBox_dev_param);
        ...
        else if (device->task_kind == DEVICE_TASK_NFC) {
            reset_window_option(false);
            this->Controls->Add(this->grpBox_ButtonTest);
            this->Controls->Add(this->grpBox_nfc);
            option_is_on = 5;
        }
        else if (device->task_kind == DEVICE_TASK_IR) {
            reset_window_option(false);
            this->Controls->Add(this->grpBox_IR);
            this->Controls->Add(this->grpBox_IRSettings);
    ...
    private: System::Void rumbleTaskWorker(System::Object^ requestObject) {
        RumbleTaskRequest^ request = safe_cast<RumbleTaskRequest^>(requestObject);
        device_context *device = static_cast<device_context *>(request->nativeDevice.ToPointer());
        set_thread_device_context(device);
        int result = 1;
        try {
            pin_ptr<byte> loaded = &request->loadedFile[0];
            pin_ptr<byte> converted = &request->convertedFile[0];
            result = play_hd_rumble_file(request->fileType, (u16)request->sampleRate, request->samples,
                request->loopStart, request->loopEnd, request->loopWait, request->loopTimes,
                loaded, converted);
        }
        catch (...) {
            result = 1;
        }
        device->task_result = result == 0 ? 1 : (result == 2 ? 2 : 3);
        if (result == 0)
            device->task_progress = 100;
        device->task_kind = DEVICE_TASK_NONE;
        device->task_total_ms = 0;
        set_thread_device_context(nullptr);
    }

    private: System::Void spiBackupTaskWorker(System::Object^ requestObject) {
        SpiBackupTaskRequest^ request = safe_cast<SpiBackupTaskRequest^>(requestObject);
        device_context *device = static_cast<device_context *>(request->nativeDevice.ToPointer());
        set_thread_device_context(device);
        int result = 1;
        String^ partialPath = request->fileName + L".partial";
        try {
            msclr::interop::marshal_context context;
            result = dump_spi(context.marshal_as<std::string>(partialPath).c_str());
            if (result == 0 && !cancel_spi_dump) {
                FileInfo^ partialInfo = gcnew FileInfo(partialPath);
                if (!partialInfo->Exists || partialInfo->Length != 524288) {
                    result = 1;
                }
                else {
                    if (File::Exists(request->fileName))
                        File::Delete(request->fileName);
                    File::Move(partialPath, request->fileName);
                    writeBackupChecksum(request->fileName);
                    send_rumble();
                }
            }
        }
        ...
        if (cancel_spi_dump)
            device->task_result = 2;
        else
            device->task_result = result == 0 ? 1 : 3;
        if (result == 0 && !cancel_spi_dump)
            device->task_progress = 100;
        device->task_kind = DEVICE_TASK_NONE;
        device->task_total_ms = 0;
    ...
    private: Control^ findDeviceControl(String^ controlName) {
        for each (Control^ root in deviceStateRoots()) {
            if (root->Name == controlName)
                return root;
            array<Control^>^ matches = root->Controls->Find(controlName, true);
            if (matches->Length > 0)
                return matches[0];
        }
        return nullptr;
    ...
    public: System::Void setSessionControlText(int slot, String^ controlName, String^ value, bool append) {
        if (this->InvokeRequired) {
            this->BeginInvoke(gcnew SessionControlTextDelegate(this, &FormJoy::setSessionControlText),
                gcnew array<Object^> { slot, controlName, value, append });
            return;
        }
        if (slot < 0 || slot >= 4)
            return;
        ...
        if (append && state->controlValues->TryGetValue(controlName, oldValue))
            combined = safe_cast<String^>(oldValue) + value;
        state->controlValues[controlName] = combined;

        if (slot == activeSlot) {
            Control^ control = findDeviceControl(controlName);
            if (control)
                control->Text = combined;
        }
    }

    public: System::Void setSessionIRImage(int slot, Image^ image) {
        if (slot < 0 || slot >= 4)
            return;
        Image^ previousImage = deviceUiStates[slot]->irImage;
        deviceUiStates[slot]->irImage = image;
        if (previousImage && slot != activeSlot)
            delete previousImage;
    }

    private: System::Void genericDeviceTaskWorker(System::Object^ requestObject) {
        DeviceTaskRequest^ request = safe_cast<DeviceTaskRequest^>(requestObject);
        device_context *device = static_cast<device_context *>(request->nativeDevice.ToPointer());
        set_thread_device_context(device);
        int result = 1;
        try {
            if (request->taskKind == DEVICE_TASK_BUTTON_TEST)
                result = button_test();
            else if (request->taskKind == DEVICE_TASK_NFC)
                result = nfc_tag_info();
        }
        catch (...) {
            result = 1;
        }
        if (request->taskKind == DEVICE_TASK_NFC) {
            device->enable_nfc_scanning_state = false;
            if (result > 0)
                setSessionControlText(device->slot, L"txtBox_nfcUid",
                    String::Format(L"NFC error {0}", result), false);
        }
        if (request->taskKind == DEVICE_TASK_BUTTON_TEST)
            device->enable_button_test_state = false;
        device->task_result = device->cancel_requested ? 2 : (result == 0 ? 1 : 3);
        device->task_kind = DEVICE_TASK_NONE;
        device->task_total_ms = 0;
        set_thread_device_context(nullptr);
    }

    private: System::Void irTaskWorker(System::Object^ requestObject) {
        IrTaskRequest^ request = safe_cast<IrTaskRequest^>(requestObject);
        device_context *device = static_cast<device_context *>(request->nativeDevice.ToPointer());
        ir_image_config *config = static_cast<ir_image_config *>(request->nativeConfig.ToPointer());
        set_thread_device_context(device);
        int result = 1;
        try {
            result = ir_sensor(*config);
        }
        catch (...) {
            result = 1;
        }
        ...
        device->task_total_ms = 0;
        if (result == 0)
            setSessionControlText(device->slot, L"lbl_IRStatus",
                request->photo ? L"Status: Photo saved" : L"Status: Stream stopped", false);
        else
            setSessionControlText(device->slot, L"lbl_IRStatus",
                String::Format(L"Status: IR error {0}", result), false);
        set_thread_device_context(nullptr);
    }
    ...
    private: System::Void btn_makeSPIBackup_Click(System::Object^  sender, System::EventArgs^  e) {
        device_context *device = current_device_context();
        if (device && device->task_kind != DEVICE_TASK_NONE) {
            MessageBox::Show(L"This controller is busy with " + taskTypeName(device->task_kind) + L".",
                L"Controller busy", MessageBoxButtons::OK, MessageBoxIcon::Information);
            return;
        }
        if (check_if_connected())
        ...
        unsigned char device_info[10];
        memset(device_info, 0, sizeof(device_info));
        if (get_device_info(device_info) != 0) {
            MessageBox::Show(L"Could not read the selected controller identity. Backup was not started.",
                L"Identity read failed", MessageBoxButtons::OK, MessageBoxIcon::Stop);
            return;
        }
        ...
        filename += String::Format("{0:X2}{1:X2}{2:X2}{3:X2}{4:X2}{5:X2}",
            device_info[4], device_info[5], device_info[6], device_info[7], device_info[8], device_info[9]);
        filename += L"_" + DateTime::Now.ToString(L"yyyyMMdd_HHmmss") + L".bin";
        ...
        if (do_backup) {
            ...
            this->label_progress->Text = L"0%";
            this->btn_spiCancel->Enabled = true;

            SpiBackupTaskRequest^ request = gcnew SpiBackupTaskRequest();
            request->nativeDevice = IntPtr(device);
            request->fileName = path;
            device->cancel_requested = false;
            device->task_progress = 0;
            device->task_result = 0;
            device->task_started_ms = GetTickCount64();
            device->task_total_ms = 0;
            device->task_kind = DEVICE_TASK_SPI_BACKUP;

            Thread^ worker = gcnew Thread(gcnew ParameterizedThreadStart(this, &FormJoy::spiBackupTaskWorker));
            worker->IsBackground = true;
            worker->Name = String::Format(L"SPI Backup Device {0}", device->slot + 1);
            deviceWorkers[device->slot] = worker;
    ...
    {
        if (handler_close == 2) {
            MessageBox::Show(L"A full restore is still writing controller data. Wait for it to finish before exiting.",
                L"Full restore in progress", MessageBoxButtons::OK, MessageBoxIcon::Warning);
            e->Cancel = true;
            return;
        }
        int runningTasks = 0;
        ...
        if (handler_close != 0 || runningTasks > 0) {
            String^ warning = handler_close == 1
                    ? L"An SPI backup is still running."
                    : String::Format(L"{0} controller task(s) are still running.", runningTasks);
            if (MessageBox::Show(warning + L"\n\nStop the tasks and exit?", L"Tasks in progress",
        ...
        for (int i = 0; i < 4; ++i) {
            if (deviceWorkers && deviceWorkers[i] && deviceWorkers[i]->IsAlive)
                deviceWorkers[i]->Join();
        }
        e->Cancel = false;
    }
    ...
    private: System::Void btn_loadSPIBackup_Click(System::Object^  sender, System::EventArgs^  e) {
            ...
            (fileStream = openFileDialog1->OpenFile()) != nullptr)
        {
            ...
            integrity_check = validateBackupChecksum(openFileDialog1->FileName, this->backup_spi);
            if (integrity_check)
                validated_backup_hash = calculateBytesSha256(this->backup_spi);
            ...
            if (get_device_info(mac_addr) != 0) {
                MessageBox::Show(L"Could not read the selected controller identity. No restore operation was enabled.",
                    L"Identity read failed", MessageBoxButtons::OK, MessageBoxIcon::Stop);
                this->txtBox_fileLoaded->Text = L"No file loaded";
                this->comboBox_rstOption->Visible = false;
                this->btn_restore->Visible = false;
            ...
            if (!mac_check) {
                ...
                this->btn_restore->Enabled = true;
                this->lbl_rstDisclaimer->Visible = false;
                allow_full_restore = integrity_check;
            }
        }
    }
    ...
    private: System::Void comboBox_rstOption_SelectedIndexChanged(System::Object^  sender, System::EventArgs^  e) {
        ...
        if (this->comboBox_rstOption->SelectedIndex == 1) {
            ...
            this->lbl_rstDesc->Text = L"This restores the Factory and User configuration.\n\n" +
                ...
                L"the full restore requires a same-device backup with a valid SHA-256 checksum.";
    ...
    private: System::Void btn_restore_Click(System::Object^  sender, System::EventArgs^  e) {
        ...
        this->deviceStripPanel->Enabled = false;
        this->taskOverviewPanel->Enabled = false;
        set_led_busy();
        int error = 0;
        if (this->comboBox_rstOption->SelectedIndex == 0) {
            ...
            if (!loadedBackupMatchesCurrentController() || !loadedBackupHasValidChecksum()) {
                MessageBox::Show(L"Serial-number restore requires a verified backup of this exact controller.",
                    L"Safety check failed", MessageBoxButtons::OK, MessageBoxIcon::Stop);
                error = 1;
            }
            else if (MessageBox::Show(L"The serial number will be restored with the backup values!\n\nAre you sure you want to continue?",
                L"Warning!", MessageBoxButtons::YesNo, MessageBoxIcon::Warning) == System::Windows::Forms::DialogResult::Yes)
            {
            ...
            if (!loadedBackupMatchesCurrentController() || !loadedBackupHasValidChecksum()) {
                MessageBox::Show(L"Calibration restore requires a verified backup of this exact controller.",
                    L"Safety check failed", MessageBoxButtons::OK, MessageBoxIcon::Stop);
                error = 1;
            }
            else if (MessageBox::Show(L"The selected user calibration will be restored from the backup!\n\n" +
            ...
            bool identity_matches = loadedBackupMatchesCurrentController() && allow_full_restore &&
                loadedBackupHasValidChecksum();
            if (!identity_matches) {
                MessageBox::Show(L"Full Restore blocked: the verified backup identity no longer matches the selected controller.",
                    L"Safety check failed", MessageBoxButtons::OK, MessageBoxIcon::Stop);
                error = 1;
            }
            else if (MessageBox::Show(L"This will do a full restore of the Factory configuration and User calibration!\n\n" +
                L"Are you sure you want to continue?",
                L"Warning!", MessageBoxButtons::YesNo, MessageBoxIcon::Warning) == System::Windows::Forms::DialogResult::Yes)
            {
                handler_close = 2;
                ...
                this->btn_writeStickParams->Enabled = false;
                this->btn_writeUserCal->Enabled = false;
                device_context *restoreDevice = current_device_context();
                restoreDevice->task_kind = DEVICE_TASK_RESTORE;
                restoreDevice->task_progress = 0;
                restoreDevice->task_result = 0;
                restoreDevice->task_started_ms = GetTickCount64();
                restoreDevice->task_total_ms = 0;
                ...
                for (int i = 0x00; i < 0x1000; i = i + 0x10) {
                    restoreDevice->task_progress = (i * 50) / 0x1000;
                ...
                for (int i = 0x00; i < 0x1000; i = i + 0x10) {
                    restoreDevice->task_progress = 50 + (i * 50) / 0x1000;
                    memset(full_restore_data, 0, 0x10);
                    for (int j = 0; j < 0x10; j++)
                ...
                if (error == 0) {
                    restoreDevice->task_progress = 100;
                    // Set shipment
                    unsigned char custom_cmd[44];
                    memset(custom_cmd, 0, sizeof(custom_cmd));
                    custom_cmd[0] = 0x01;
                    custom_cmd[5] = 0x08;
                    custom_cmd[6] = 0x01;
                    error = send_internal_command(custom_cmd);
                    // Clear pairing info
                    memset(custom_cmd, 0, sizeof(custom_cmd));
                    custom_cmd[0] = 0x01;
                    custom_cmd[5] = 0x07;
                ...
                restoreDevice->task_result = error == 0 ? 1 : 3;
                restoreDevice->task_kind = DEVICE_TASK_NONE;
                restoreDevice->task_total_ms = 0;
                handler_close = 0;
                this->grpBox_Color->Visible = true;
        ...
        this->deviceStripPanel->Enabled = true;
        this->taskOverviewPanel->Enabled = true;
    }
    ...
    private: System::Void textBox_loop_Validating(System::Object^ sender, CancelEventArgs^ e)
    {
        ...
        bool cancel = false;
        int number = -1;
        if (Int32::TryParse(test->Text, NumberStyles::Integer, CultureInfo::InvariantCulture, number) &&
            number >= 0 && number <= 999)
        {
            cancel = false;
        }
    ...
    private: System::Void textBox_loop_Validated(System::Object^ sender, System::EventArgs^ e)
    {
        ...
        if ((vib_file_type == 3 || vib_file_type == 4) &&
            Int32::TryParse(test->Text, NumberStyles::Integer, CultureInfo::InvariantCulture, loops)) {
            unsigned long long totalSamples = vib_loop_start +
                (unsigned long long)(vib_loop_end - vib_loop_start) * (1 + loops) +
                (vib_samples - vib_loop_end) +
                (unsigned long long)vib_loop_wait * (1 + loops);
            this->label_samples->Text = String::Format(L"Samples: {0}  |  Total: {1}",
                vib_samples, formatTaskTime(totalSamples * vib_sample_rate));
        }
    }
        ...
        if (handle_ok != 3) {
                ...
                MessageBoxButtons::YesNo, MessageBoxIcon::Warning) == System::Windows::Forms::DialogResult::Yes)
            {
                    ...
                    MessageBoxButtons::YesNo, MessageBoxIcon::Warning) == System::Windows::Forms::DialogResult::Yes)
                {
                    ...
                    if (get_spi_data(0x6000, 0x10, spi_sn) != 0) {
                        MessageBox::Show(L"Could not read the current serial number. No data was written.",
                            L"Read failed", MessageBoxButtons::OK, MessageBoxIcon::Stop);
                        return;
                    }
                    for (int i = 0; i < 3; i++) {
                    ...
                    if (get_spi_data(0xF000, 0x1, sn_backup) != 0) {
                        MessageBox::Show(L"Could not read the serial-number backup area. No data was written.",
                            L"Read failed", MessageBoxButtons::OK, MessageBoxIcon::Stop);
                        return;
                    }
                    ...
                    if (error != 0) {
                        MessageBox::Show(L"Could not verify the serial-number backup. The current serial number was not changed.",
                            L"Backup failed", MessageBoxButtons::OK, MessageBoxIcon::Stop);
                        return;
                    }
                    Sleep(100);
    ...
    private: System::Void btn_restoreSN_Click(System::Object^  sender, System::EventArgs^  e) {
        ...
        if (handle_ok != 3) {
            if (MessageBox::Show(L"Do you really want to restore it from the S/N backup inside your controller\'s SPI?\n\nYou can also choose to restore it from a SPI backup you previously made, through the main Restore option.",
                ...
                if (get_spi_data(0xF000, 0x10, spi_sn) != 0) {
                    MessageBox::Show(L"Could not read the serial-number backup. No data was written.",
                        L"Read failed", MessageBoxButtons::OK, MessageBoxIcon::Stop);
                    return;
                }
                Sleep(100);
    ...
    private: System::Void btn_loadVib_Click(System::Object^  sender, System::EventArgs^  e) {
        ...
        {    
            ...
            bool valid_rumble_file = false;
            int file_length = this->vib_loaded_file->Length;
            if (file_length >= 10 && this->vib_loaded_file[0] == file_magic[0] &&
                this->vib_loaded_file[1] == file_magic[1] &&
                this->vib_loaded_file[2] == file_magic[2] &&
                this->vib_loaded_file[3] == file_magic[3]) {
                u16 rate = (this->vib_loaded_file[4] << 8) | this->vib_loaded_file[5];
                u32 samples = readBigEndian32(this->vib_loaded_file, 6);
                valid_rumble_file = rate > 0 && samples > 0 &&
                    0x0AULL + (u64)samples * 4ULL <= (u64)file_length;
            }
            else if (file_length >= 12 && this->vib_loaded_file[4] == file_magic[6]) {
                u16 frequency = this->vib_loaded_file[6] | (this->vib_loaded_file[7] << 8);
                int header_size = 0;
                u32 data_size = 0;
                u32 loop_start = 0;
                u32 loop_end = 0;
                if (this->vib_loaded_file[0] == file_magic[4]) {
                    header_size = 0x0C;
                    data_size = readLittleEndian32(this->vib_loaded_file, 0x08);
                }
                else if (this->vib_loaded_file[0] == file_magic[5] && file_length >= 0x14) {
                    header_size = 0x14;
                    data_size = readLittleEndian32(this->vib_loaded_file, 0x10);
                    loop_start = readLittleEndian32(this->vib_loaded_file, 0x08);
                    loop_end = readLittleEndian32(this->vib_loaded_file, 0x0C);
                }
                else if (this->vib_loaded_file[0] == file_magic[7] && file_length >= 0x18) {
                    header_size = 0x18;
                    data_size = readLittleEndian32(this->vib_loaded_file, 0x14);
                    loop_start = readLittleEndian32(this->vib_loaded_file, 0x08);
                    loop_end = readLittleEndian32(this->vib_loaded_file, 0x0C);
                }
                u32 samples = data_size / 4;
                bool loop_valid = header_size <= 0x0C || loop_start <= loop_end && loop_end <= samples;
                valid_rumble_file = header_size > 0 && frequency > 0 && frequency <= 1000 &&
                    data_size > 0 && (data_size % 4) == 0 && loop_valid &&
                    (u64)header_size + data_size <= (u64)file_length;
            }
            if (!valid_rumble_file) {
                vib_file_type = 0;
                this->label_vib_loaded->Text = L"Type: Invalid or truncated file";
                this->btn_vibPlay->Enabled = false;
                this->groupBox_vib_eq->Visible = false;
                MessageBox::Show(L"The HD Rumble file header, timing, sample data, or loop range is invalid.",
                    L"Invalid vibration file", MessageBoxButtons::OK, MessageBoxIcon::Stop);
                return;
            }
    ...
    private: System::Void btn_vibPlay_Click(System::Object^  sender, System::EventArgs^  e) {
        ...
        if (device && device->task_kind == DEVICE_TASK_RUMBLE) {
            device->cancel_requested = true;
            this->btn_vibPlay->Text = L"Stopping...";
            this->btn_vibPlay->Enabled = false;
            return;
        }
        if (device && device->task_kind != DEVICE_TASK_NONE) {
        ...
        if (check_if_connected())
            return;
        if (!this->vib_loaded_file || !this->vib_file_converted || vib_file_type == 0) {
            MessageBox::Show(L"Load a valid HD Rumble file first.", L"No vibration file",
        ...
        this->groupBox_vib_eq->Enabled = false;
        this->btn_vibPlay->Text = L"Stop";

        saveActiveDeviceState();
        RumbleTaskRequest^ request = gcnew RumbleTaskRequest();
        request->nativeDevice = IntPtr(device);
        request->loadedFile = safe_cast<array<byte>^>(this->vib_loaded_file->Clone());
        request->convertedFile = safe_cast<array<byte>^>(this->vib_file_converted->Clone());
        request->fileType = vib_file_type;
        request->sampleRate = vib_sample_rate;
        request->samples = vib_samples;
        request->loopStart = vib_loop_start;
        request->loopEnd = vib_loop_end;
        request->loopWait = vib_loop_wait;
        request->loopTimes = vib_loop_times;

        unsigned long long totalSamples = vib_samples;
        if (vib_file_type == 3 || vib_file_type == 4) {
            totalSamples = vib_loop_start +
                (unsigned long long)(vib_loop_end - vib_loop_start) * (1 + vib_loop_times) +
                (vib_samples - vib_loop_end) +
                (unsigned long long)vib_loop_wait * (1 + vib_loop_times);
        }
        this->label_samples->Text = String::Format(L"Samples: {0}  |  Total: {1}",
            vib_samples, formatTaskTime(totalSamples * vib_sample_rate));
        if (totalSamples * vib_sample_rate > 30ULL * 60ULL * 1000ULL &&
            MessageBox::Show(L"This playback will run for more than 30 minutes. Continue?",
                L"Long playback", MessageBoxButtons::YesNo, MessageBoxIcon::Warning,
                MessageBoxDefaultButton::Button2) != System::Windows::Forms::DialogResult::Yes) {
            this->btn_vibPlay->Text = L"Play";
            this->btn_loadVib->Enabled = true;
            this->groupBox_vib_eq->Enabled = vib_file_type != 1;
        ...
        device->task_result = 0;
        device->task_started_ms = GetTickCount64();
        device->task_total_ms = totalSamples * vib_sample_rate;
        device->task_kind = DEVICE_TASK_RUMBLE;

        Thread^ worker = gcnew Thread(gcnew ParameterizedThreadStart(this, &FormJoy::rumbleTaskWorker));
        worker->IsBackground = true;
        worker->Name = String::Format(L"HD Rumble Device {0}", device->slot + 1);
        deviceWorkers[device->slot] = worker;
        worker->Start(request);
    ...
    private: System::Void btn_runBtnTest_Click(System::Object^  sender, System::EventArgs^  e) {
        ...
        if (device && device->task_kind == DEVICE_TASK_BUTTON_TEST) {
            device->cancel_requested = true;
            enable_button_test = false;
            this->btn_runBtnTest->Text = L"Stopping...";
            this->btn_runBtnTest->Enabled = false;
            return;
        }
        if (device && device->task_kind != DEVICE_TASK_NONE) {
            MessageBox::Show(L"This controller is busy with " + taskTypeName(device->task_kind) + L".",
        ...
        device->task_started_ms = GetTickCount64();
        device->task_total_ms = 0;
        device->task_kind = DEVICE_TASK_BUTTON_TEST;

        DeviceTaskRequest^ request = gcnew DeviceTaskRequest();
        request->nativeDevice = IntPtr(device);
        request->taskKind = DEVICE_TASK_BUTTON_TEST;
        Thread^ worker = gcnew Thread(gcnew ParameterizedThreadStart(this, &FormJoy::genericDeviceTaskWorker));
        worker->IsBackground = true;
        worker->Name = String::Format(L"Button Test Device {0}", device->slot + 1);
        deviceWorkers[device->slot] = worker;
        worker->Start(request);
    ...
    private: System::Void btn_spiCancel_Click(System::Object^  sender, System::EventArgs^  e) {
        ...
        this->btn_spiCancel->Enabled = false;
        this->label_progress->Text = L"Stopping...";
    }
    ...
    private: System::Void toolStripBtn_Disconnect_Click(System::Object^  sender, System::EventArgs^  e) {
        ...
        if (!device)
            return;
        if (device->task_kind != DEVICE_TASK_NONE) {
            MessageBox::Show(L"Stop the running task before disconnecting this controller.",
                L"Controller busy", MessageBoxButtons::OK, MessageBoxIcon::Information);
            return;
        }
        unsigned char custom_cmd[44];
        ...
        custom_cmd[5] = 0x06;
        custom_cmd[6] = 0x00;
        send_internal_command(custom_cmd);
        ...
        showNoDeviceSelected();
        for (int i = 0; i < 4; ++i) {
            device_context *candidate = get_device_slot(i);
    ...
    private: System::Int32 prepareSendIRConfig(bool startNewConfig) {
        ...
        device_context *device = current_device_context();
        if (device) {
            device->ir_image_width = ir_image_width;
            device->ir_image_height = ir_image_height;
            device->ir_exposure = (int)this->numeric_IRExposure->Value;
            if (this->radioBtn_IRColorHeat->Checked)
                device->ir_color_mode = 0;
            else if (this->radioBtn_IRColorGrey->Checked)
                device->ir_color_mode = 1;
            else if (this->radioBtn_IRColorGreen->Checked)
                device->ir_color_mode = 2;
            else
                device->ir_color_mode = 3;
        }
        ...
        // Initialize camera
        if (startNewConfig) {
            IrTaskRequest^ request = gcnew IrTaskRequest();
            request->nativeDevice = IntPtr(device);
            request->nativeConfig = IntPtr(new ir_image_config(ir_new_config));
            request->photo = !enable_IRVideoPhoto;
            device->cancel_requested = false;
            device->task_progress = 0;
            device->task_result = 0;
            device->task_started_ms = GetTickCount64();
            device->task_total_ms = 0;
            device->task_kind = DEVICE_TASK_IR;

            Thread^ worker = gcnew Thread(gcnew ParameterizedThreadStart(this, &FormJoy::irTaskWorker));
            worker->IsBackground = true;
            worker->Name = String::Format(L"IR Device {0}", device->slot + 1);
            deviceWorkers[device->slot] = worker;
            worker->Start(request);
        }
        // Change camera configuration
            ...
            EnterCriticalSection(&device->ir_config_lock);
            device->pending_ir_config = ir_new_config;
            LeaveCriticalSection(&device->ir_config_lock);
            device->pending_ir_config_update = true;
            res = 0;
        }
    ...
    public: System::Void setIRPictureWindow(u8* buf_image, bool ir_video_photo) {
        device_context *device = current_device_context();
        if (!device || device->ir_image_width <= 0 || device->ir_image_height <= 0)
            return;
        int imageWidth = device->ir_image_width;
        int imageHeight = device->ir_image_height;
        int colorMode = device->ir_color_mode;
        Bitmap^ MyImage = gcnew Bitmap(imageWidth, imageHeight, System::Drawing::Imaging::PixelFormat::Format24bppRgb);
        int buf_pos = 0;

        // Skip slow SetPixel(). Reduce latency pixel set latency from 842us -> 260ns.
        System::Drawing::Imaging::BitmapData^ bmd = MyImage->LockBits(System::Drawing::Rectangle(0, 0, imageWidth, imageHeight), System::Drawing::Imaging::ImageLockMode::WriteOnly, MyImage->PixelFormat);
        int PixelSize = 3;
        ...
        if (!enable_IRVideoPhoto) {
            String^ photoName = String::Format(L"IRcamera_device{0}_{1}.png", device->slot + 1,
                DateTime::Now.ToString(L"yyyyMMdd_HHmmss"));
            String^ photoPath = Path::Combine(Path::GetDirectoryName(Application::ExecutablePath), photoName);
            rotatedImage->Save(photoPath, System::Drawing::Imaging::ImageFormat::Png);
        }
        ...
        setSessionIRImage(device->slot, resizedImage);
    }
    ...
    private: System::Void btn_getImage_Click(System::Object^  sender, System::EventArgs^  e) {
        ...
        this->btn_getIRImage->Enabled  = false;
        enable_IRVideoPhoto = false;
        int res = prepareSendIRConfig(true);
        if (res != 0) {
            this->lbl_IRStatus->Text = String::Format(L"Status: Configuration error {0}", res);
            this->btn_getIRStream->Enabled = true;
            this->btn_getIRImage->Enabled = true;
        }
    }
    ...
    private: System::Void btn_getVideo_Click(System::Object^  sender, System::EventArgs^  e) {
        ...
        if (device && device->task_kind == DEVICE_TASK_IR) {
            device->cancel_requested = true;
            enable_IRVideoPhoto = false;
            this->btn_getIRStream->Text = L"Stopping...";
            this->btn_getIRStream->Enabled = false;
            return;
        }
        if (device && device->task_kind != DEVICE_TASK_NONE) {
    ...
    private: System::Void btn_NFC_Click(System::Object^  sender, System::EventArgs^  e) {
        ...
        if (device && device->task_kind == DEVICE_TASK_NFC) {
            device->cancel_requested = true;
            enable_NFCScanning = false;
            this->btn_NFC->Text = L"Stopping...";
            this->btn_NFC->Enabled = false;
            return;
        }
        if (device && device->task_kind != DEVICE_TASK_NONE) {
            MessageBox::Show(L"This controller is busy with " + taskTypeName(device->task_kind) + L".",
        ...
        device->task_started_ms = GetTickCount64();
        device->task_total_ms = 0;
        device->task_kind = DEVICE_TASK_NFC;

        DeviceTaskRequest^ request = gcnew DeviceTaskRequest();
        request->nativeDevice = IntPtr(device);
        request->taskKind = DEVICE_TASK_NFC;
        Thread^ worker = gcnew Thread(gcnew ParameterizedThreadStart(this, &FormJoy::genericDeviceTaskWorker));
        worker->IsBackground = true;
        worker->Name = String::Format(L"NFC Device {0}", device->slot + 1);
        deviceWorkers[device->slot] = worker;
        worker->Start(request);
    }
    ...
    private: System::Void btn_refreshUserCal_Click(System::Object^  sender, System::EventArgs^  e) {
        ...
        int readResult = get_spi_data(0x8010, sizeof(user_stick_cal), user_stick_cal);
        Sleep(100);
        if (readResult == 0)
            readResult = get_spi_data(0x8026, sizeof(user_sensor_cal), user_sensor_cal);
        Sleep(100);
        if (readResult == 0)
            readResult = get_spi_data(0x6089, sizeof(stick_model_main_left), stick_model_main_left);
        if (readResult == 0 && handle_ok == 3) {
            Sleep(100);
            readResult = get_spi_data(0x609B, sizeof(stick_model_pro_right), stick_model_pro_right);
        }
        if (readResult != 0) {
            grpBox_leftStickUCal->Enabled = false;
            grpBox_rightStickUCal->Enabled = false;
            ...
            this->btn_writeStickParams->Enabled = false;
            this->btn_writeUserCal->Enabled = false;
            MessageBox::Show(L"Calibration data could not be read completely. Writing remains disabled.",
                L"SPI read failed", MessageBoxButtons::OK, MessageBoxIcon::Stop);
            return;
        }

        unsigned char calibrationDeviceInfo[10];
        memset(calibrationDeviceInfo, 0, sizeof(calibrationDeviceInfo));
        if (get_device_info(calibrationDeviceInfo) != 0) {
            MessageBox::Show(L"Controller identity could not be verified. Writing remains disabled.",
                L"Identity read failed", MessageBoxButtons::OK, MessageBoxIcon::Stop);
            return;
        }
        calibration_read_mac = String::Format("{0:X2}:{1:X2}:{2:X2}:{3:X2}:{4:X2}:{5:X2}",
            calibrationDeviceInfo[4], calibrationDeviceInfo[5], calibrationDeviceInfo[6],
            calibrationDeviceInfo[7], calibrationDeviceInfo[8], calibrationDeviceInfo[9]);
        ...
        calibration_read_valid = true;
        this->btn_writeStickParams->Enabled = true;
        this->btn_writeUserCal->Enabled = true;
    }


    private: bool stickCalibrationIsOrdered(NumericUpDown^ xMinus, NumericUpDown^ xCenter,
        NumericUpDown^ xPlus, NumericUpDown^ yMinus, NumericUpDown^ yCenter, NumericUpDown^ yPlus) {
        return xMinus->Value < xCenter->Value && xCenter->Value < xPlus->Value &&
            yMinus->Value < yCenter->Value && yCenter->Value < yPlus->Value;
    }

    private: System::Void btn_writeUserCal_Click(System::Object^  sender, System::EventArgs^  e) {
        if (check_if_connected())
            return;
        if (!calibrationStateMatchesCurrentController()) {
            calibration_read_valid = false;
            calibration_read_mac = L"";
            this->btn_writeStickParams->Enabled = false;
            this->btn_writeUserCal->Enabled = false;
            MessageBox::Show(L"Read and validate this controller's calibration data before writing.",
                L"Calibration write blocked", MessageBoxButtons::OK, MessageBoxIcon::Stop);
            return;
        }

        if (handle_ok != 2 && this->checkBox_enableLeftUserCal->Checked &&
            !stickCalibrationIsOrdered(this->numeric_leftUserCal_x_minus,
                this->numeric_leftUserCal_x_center, this->numeric_leftUserCal_x_plus,
                this->numeric_leftUserCal_y_minus, this->numeric_leftUserCal_y_center,
                this->numeric_leftUserCal_y_plus)) {
            MessageBox::Show(L"Left stick calibration blocked: each axis must satisfy minus < center < plus.",
                L"Invalid calibration", MessageBoxButtons::OK, MessageBoxIcon::Stop);
            return;
        }
        if (handle_ok != 1 && this->checkBox_enableRightUserCal->Checked &&
            !stickCalibrationIsOrdered(this->numeric_rightUserCal_x_minus,
                this->numeric_rightUserCal_x_center, this->numeric_rightUserCal_x_plus,
                this->numeric_rightUserCal_y_minus, this->numeric_rightUserCal_y_center,
                this->numeric_rightUserCal_y_plus)) {
            MessageBox::Show(L"Right stick calibration blocked: each axis must satisfy minus < center < plus.",
                L"Invalid calibration", MessageBoxButtons::OK, MessageBoxIcon::Stop);
            return;
        }
    ...
    private: System::Boolean check_if_connected() {
        if (handler_close != 0) {
            MessageBox::Show(L"Wait for the current restore operation to finish.",
                L"Controller busy", MessageBoxButtons::OK, MessageBoxIcon::Information);
            return true;
        }
        device_context *device = current_device_context();
        if (device && device->task_kind != DEVICE_TASK_NONE) {
            MessageBox::Show(L"This controller is busy with " + taskTypeName(device->task_kind) +
                L". Stop that task before starting another operation.",
                L"Controller busy", MessageBoxButtons::OK, MessageBoxIcon::Information);
            return true;
        }
        if (!device_connection()) {
    ...
    public: System::Void show_ntag_contents(int slot, u8* ntag_buf, u8 ntag_pages) {
        ...
        if (!String::IsNullOrEmpty(tag_type))
            setSessionControlText(slot, L"txtBox_nfcUid", tag_type, true);
        setSessionControlText(slot, L"txtBox_NFCTag", ntag_temp_string, false);
    }
};
}
